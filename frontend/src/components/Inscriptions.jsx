...
export default function Inscriptions({ mode = 'inscriptions' }) {
  ...
  // ── Validate / Reject unified handler ─
  ...
  const filtered = displayEmployees.filter(e =>
    `${e.prenom} ${e.nom} ${e.email} ${e.poste} ${e.typeContrat} ${e.modeReglement}`
      .toLowerCase()
        ...
        <div className="insc-header-actions">
          <div className="insc-search-row">
            ...
            <input
              ...
              placeholder="Rechercher un employé, un poste, une adresse..."
            />
          </div>
          {isEmployeePage && (
            ...
            {filtered.map(emp => (
              <article className="insc-card" key={emp.id}>
                <div className="insc-card-top">
                  <div
                    className="insc-avatar"
                    style={{ background: avatarColor(emp.prenom) }}
                  >
                ...
                <div className="insc-card-actions">
                  <button className="btn-voir" onClick={() => openViewModal(emp)}>
                    Détails
                  </button>
          ...
          filtered.map(emp => (
            <div className="insc-row" key={emp.id}>
              ...
              <div className="insc-actions">
                <button className="btn-voir" onClick={() => setSelectedEmployee(emp)}>
                  Voir
                </button>

                <button
                  className="btn-valider"
                  onClick={() => handleAction(emp.id, 'valider')}
                ...
                <button
                  ...
                  onClick={() => handleAction(emp.id, 'refuser')}
          ...
          >
            ...
            <div className="modal-header modal-form-header">
              <div>
                <h2 className="modal-name">
                  {modalMode === 'create' ? 'Nouvel employé' : `${selectedEmployee?.prenom || ''} ${selectedEmployee?.nom || ''}`}
                </h2>
                <p className="modal-role">
                  {modalMode === 'create'
                    ? 'Ajoutez un employé et enregistrez-le dans la base'
                    : modalMode === 'edit'
            ...
            {(modalMode === 'view') ? (
              <div className="modal-grid">
                ...
                <div className="modal-field">
                  <span>Téléphone</span>
                  <strong>{selectedEmployee.telephone || '—'}</strong>
                </div>
                <div className="modal-field">
                  <span>Contrat</span>
                  <strong>{selectedEmployee.typeContrat || 'CDI'}</strong>
                </div>
                <div className="modal-field">
                  <span>Règlement</span>
                  <strong>{selectedEmployee.modeReglement || 'Virement'}</strong>
                </div>
                <div className="modal-field">
                  <span>Soumis le</span>
                  <strong>{formatDate(selectedEmployee.createdAt || selectedEmployee.created_at)}</strong>
                </div>
                <div className="modal-field">
                  <span>Statut</span>
                  <strong>{selectedEmployee.statut?.replace('_', ' ') || 'EN ATTENTE'}</strong>
                </div>
              </div>
            ) : (
              <div className="modal-form-grid">
                ...
                <div className="form-field">
                  <label>Contrat</label>
                  <select
                    value={employeeForm.typeContrat}
                ...
                <div className="form-field">
                  ...
                  <select
                    value={employeeForm.modeReglement}
                ...
                <>
                  {selectedEmployee?.statut === 'EN_ATTENTE' && (
                    <>
                      <button
                        className="btn-valider"
                        onClick={() => handleAction(selectedEmployee.id, 'valider')}
                      >
                        ✔ Valider
                      </button>
                      <button
                        className="btn-refuser"
                        onClick={() => handleAction(selectedEmployee.id, 'refuser')}
                      >
                        ✖ Refuser
                      </button>
                    </>
                  )}
                  <button className="btn-edit" onClick={() => openEditModal(selectedEmployee)}>
                    Modifier
                  </button>
                  <button className="btn-voir" onClick={closeModal}>
                    Fermer
                  </button>
                </>
              ) : (
